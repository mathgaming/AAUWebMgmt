...
namespace ITSWebMgmt.Connectors
{
    public class SplunkConnector
    {
        ...
        public async Task<string> IsAccountADFSLockedAsync(string upn)
        {
            if (!_cache.TryGetValue("ADFSLockedAccounts", out List<string> lockedAccounts))
            {
                lockedAccounts = await GetLockedAccountsAsync();
        ...
        private async Task<List<string>> GetLockedAccountsAsync()
        {
            try
            {
                var temp = await (await GetDataAsync()).Content.ReadAsStringAsync(); // This is done with a regex, becuase i could not find a NDJSON parser
        ...
        private async Task<HttpResponseMessage> GetDataAsync()
        {
